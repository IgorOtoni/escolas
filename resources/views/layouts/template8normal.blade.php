						...
						<nav class="menu_main_nav_area">
							<ul id="menu_main" class="menu_main_nav">
								...
								foreach($menus as $menu){
									?><li class="menu-item menu-item-has-children"><a href="{{verifica_link($menu->link, $site)}}"><?php echo $menu->nome ?></a><?php
										if($submenus != null && array_key_exists($menu->id, $submenus) && count($submenus[$menu->id]) > 0){ ?>
											<ul class="sub-menu">
												<?php foreach($submenus[$menu->id] as $submenu){
													?><li class="menu-item menu-item-has-children"><a href="{{verifica_link($submenu->link, $site)}}"><?php echo $submenu->nome ?></a><?php
